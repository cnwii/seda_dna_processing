...
otu_file <- list.files(
  path=otu_dir,
  pattern = "krakenuniq_abundance_matrix.*\\.txt$",
  full.names = TRUE)
...
make_domain_family_plot <- function(ps_input, domain_name = NULL, filename) {
  ...
  if (!is.null(domain_name)) {
    keep_taxa <- taxa_names(ps_input)[tax_table(ps_input)[, "domain"] == domain_name]
    ps_domain <- prune_taxa(keep_taxa, ps_input)
  } else {
    ps_domain <- ps_input
  }
  
  ps_domain_family <- tax_glom(ps_domain, taxrank = "family")
  
  read_counts <- data.frame(
    Sample = names(sample_sums(ps_domain_family)),
    total_reads = as.numeric(sample_sums(ps_domain_family))
  ) %>%
    mutate(read_label = paste0("total reads=", scales::comma(total_reads)))
  
  ps_domain_family_RA <- microbiome::transform(ps_domain_family, "compositional")
  
  df_domain <- psmelt(ps_domain_family_RA) %>%
