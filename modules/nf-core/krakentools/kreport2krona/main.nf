process KRAKENTOOLS_KREPORT2KRONA {
    ...
    input:
    tuple val(meta), val(param_set), path(kreport)

    output:
    tuple val(meta), val(param_set), path("*.txt"), emit: txt
    ...
    def prefix = task.ext.prefix ?: "${meta.id}_k${param_set.uniq_kmer}_r${param_set.tax_reads}"
    ...
    stub:
    def prefix = task.ext.prefix ?: "${meta.id}k${param_set.uniq_kmer}_r${param_set.tax_reads}"
    def VERSION = '1.2.1' // WARN: Version information not provided by tool on CLI. Please update this string when bumping container versions.
    """
    touch ${prefix}.txt
