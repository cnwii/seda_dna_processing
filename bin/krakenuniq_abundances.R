#!/usr/bin/env Rscript

#This is a script for combining KrakenUniq outputs from multiple samples and producing species abundance matrix.

#Run this script as:

#Rscipt krakenuniq_abundance_matrix.R input_dir output_dir n_unique_kmers n_tax_reads


args = commandArgs(trailingOnly=TRUE)

input_dir<-as.character(args[1])

output_dir<-as.character(args[2])

meta_id       <- args[3]

k_value       <- args[4]
r_value       <- args[5]


#Reading and merging krakenuniq outputs from multiple samples

krakenuniq_outputs<-list.files(path=input_dir, pattern = "\\.filtered$")
df<-list()
for(i in 1:length(krakenuniq_outputs))
{
  df[[i]]<-read.delim(paste0(input_dir,"/",krakenuniq_outputs[i]),comment.char="#",header=TRUE)
  if(dim(df[[i]])[1]!=0)
  {
    df[[i]]$SAMPLE <- sub("^([^_\\.]+)(_T1)?\\..*$", "\\1", krakenuniq_outputs[i])
    #df[[i]]<-na.omit(df[[i]])
  }
  else{next}
}
merged<-Reduce(rbind,df)
merged$taxName<-trimws(as.character(merged$taxName))
print(head(merged,20))

unique_samples<-unique(merged$SAMPLE)
unique_species<-unique(merged$taxName)
unique_taxids<-unique(merged$taxID)

print(unique_samples)
print(unique_taxids)
#Computing abundance matrix
abundance_matrix<-matrix(NA,ncol=length(unique_samples),nrow=length(unique_taxids))
for(i in 1:length(unique_taxids))
{
  for(j in 1:length(unique_samples))
  {
    if(length(merged[merged$taxID==unique_taxids[i] & merged$SAMPLE==unique_samples[j],]$taxReads)==0)
    {
      abundance_matrix[i,j]<-0
    }
    else
    {
      abundance_matrix[i,j]<-merged[merged$taxID==unique_taxids[i] & merged$SAMPLE==unique_samples[j],]$taxReads
    }
  }
}
rownames(abundance_matrix)<-unique_taxids
colnames(abundance_matrix)<-unique_samples
abundance_matrix<-abundance_matrix[order(rownames(abundance_matrix)),]
print(head(abundance_matrix))

param <- unique(gsub(".*\\.(k\\d+_r\\d+)\\..*", "\\1", krakenuniq_outputs))
  #regmatches(krakenuniq_outputs[i], regexpr("k\\d+_r\\d+", krakenuniq_outputs[i]))

system(paste0("mkdir ",output_dir))
write.table(abundance_matrix,file=paste0(output_dir,"/krakenuniq_abundance_matrix_", param, ".txt"),col.names=TRUE,row.names=TRUE,quote=FALSE,sep="\t")
write.table(unique_taxids,file=paste0(output_dir,"/unique_species_taxid_list_", param, ".txt"),col.names=FALSE,row.names=FALSE,quote=FALSE,sep="\t")